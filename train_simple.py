...
for chromosome, chr_len in zip(chr_list, chr_length):
    ...
    n_list_index = torch.load(save_path + '/negative_img_m(i)d.index' + '.pt')
    for i in range(len(p_list_index)):
        p_list_index[i] = p_list_index[i] + ap_list_index[-1]
    for i in range(len(n_list_index)):
        n_list_index[i] = n_list_index[i] + an_list_index[-1]
    ap_list_index = ap_list_index + p_list_index[1:]
    # set_trace()
    an_list_index = an_list_index + n_list_index[1:]
    # positive_img_i_list = torch.load(save_path + '/positive_img_m(i)d' + '.pt')
    # negative_img_i_list = torch.load(save_path + '/negative_img_m(i)d' + '.pt')
    print("img end")
    # img/positive_cigar_img
    # print("cigar start")
    # positive_cigar_img = torch.load(data_dir + 'image/' + chromosome + '/positive_cigar_img' + '.pt') # 4
    ...
    # length = len(positive_img_i_list)
...
torch.save(ap_list_index, data_dir + '/all_p_list.index' + '.pt')
torch.save(an_list_index, data_dir + '/all_n_list.index' + '.pt')